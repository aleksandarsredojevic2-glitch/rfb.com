...
function getRoomList() {
    return Object.keys(rooms).map(id => {
        ...
        return {
            ...
            hostName: rooms[id].hostName || 'Guest'
        };
    });
}
...
function createRoom(roomId, adminId, hostName) {
...
wss.on('connection', (ws) => {
    ...
    ws.on('message', (message) => {
        ...
        if (data.type === 'join-room') {
            ...
            const hostName = data.name || players[myId].name || "Guest";
            players[myId].name = hostName;
            createRoom(newRoomId, myId, hostName);
            ...
            if (room && room.adminId === myId) {
                ...
                room.goalLimit = (!isNaN(noviLimit) && noviLimit > 0 && noviLimit <= 5) ? noviLimit : 5;
            ...
            // Admin može ručno da prekine meč PRE nego što se dostigne broj golova za pobedu.
            let roomId = players[myId].roomId;
            let room = rooms[roomId];

            if (room && room.adminId === myId && room.gameState === 'PLAYING') {
                broadcastToRoom(roomId, { type: 'game-stopped' });
            ...
            // room.ballCarrierId je VEĆ tačno, sveže ažurirano svaki tick od strane
            // dribbling logike, i garantovano sinhronizovano sa stvarnim pozicijama
            // (pouzdanije nego da ovde ponovo računamo udaljenost).
            if (p && p.body && room && room.ballCarrierId === myId) {
                // AIM: PRAVAC suta se računa od LOPTE ka SREDINI PROTIVNIČKOG GOLA,
                // ne od pravca kretanja igrača. Crveni napadaju desni gol, plavi levi
                // (isti raspored kao u pravilima za gol/korner/gol-aut iznad).
                const targetX = p.team === 'red' ? FIELD_RIGHT : FIELD_LEFT;
                const targetY = WORLD_HEIGHT / 2;
                let dx = targetX - room.puck.position.x;
                let dy = targetY - room.puck.position.y;
                let distance = Math.sqrt(dx * dx + dy * dy);

                if (distance > 0) {
                    const SHOOT_SPEED = 24; // fiksna, jaka jačina
            ...
            // Dodavanje (X ili Space) - fiksna SREDNJA jačina, ide u pravcu kretanja
            // (bez aim-a ka golu - to je samo za šut).
            let p = players[myId];
            let room = rooms[p.roomId];
            if (p && p.body && room && room.ballCarrierId === myId) {
                ...
                let distance = Math.sqrt(dx * dx + dy * dy);
                if (distance > 0) {
                    const PASS_SPEED = 14; // fiksna, srednja jačina

                    Matter.Body.setVelocity(room.puck, { x: (dx / distance) * PASS_SPEED, y: (dy / distance) * PASS_SPEED });
                    // Kraći release period nego kod suta - dodavanje je namerno "brže u igri"
                    room.releaseUntil = Date.now() + 250;
                    room.ballCarrierId = null;
                }
            }
...
setInterval(() => {
    for (let roomId in rooms) {
        ...
        if (room.gameState === 'PLAYING' && !room.isResetting) {
            ...
            if (p.x <= FIELD_LEFT) {
                if (inGoalY) {
                    ...
                    room.score.teamBlue++;
                    handleGoalScored(roomId, room, 'blue');
                ...
                if (inGoalY) {
                    ...
                    room.score.teamRed++;
                    handleGoalScored(roomId, room, 'red');
                ...
                // Aut dobija tim SUPROTAN od onog ko je loptu poslednji dodirnuo.
                ...
                let throwTeamLabel = room.lastToucherTeam === 'red' ? 'plave'
                                    : room.lastToucherTeam === 'blue' ? 'crvene'
                                    : null;
                let text = throwTeamLabel ? `🥅 Aut za ${throwTeamLabel}!` : '🥅 Aut!';
                broadcastToRoom(roomId, { type: 'field-event', text: text });
            }
        }
        ...
        broadcastToRoom(roomId, { type: 'update', puck: { x: room.puck.position.x, y: room.puck.position.y }, players: playerList, score: room.score, adminId: room.adminId, goalLimit: room.goalLimit });
    }
}, 1000 / 60);

function handleGoalScored(roomId, room, scoringTeam) {
    // Obavesti klijente KO je dao gol - da bi mogli da prikažu "GOAAAAAAAAAL!!!!!" baner
    broadcastToRoom(roomId, { type: 'goal-scored', team: scoringTeam, score: room.score });
